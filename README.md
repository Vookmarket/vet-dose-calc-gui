...
### 5. 設定
- Gemini APIキーのGUIからの登録・管理
- APIキーはローカルファイルに保存（環境変数設定不要）
...
- suggest機能使用時: Gemini APIキー（GUI設定画面 or 環境変数 `GEMINI_API_KEY`）
...
薬剤提案（suggest）機能を使うには、Gemini APIキーが必要です。以下のいずれかの方法で設定してください。

**方法1: GUI設定画面から（推奨）**

1. アプリ起動後、サイドバーで「設定」を選択
2. APIキー入力欄にキーを入力し「保存」をクリック
3. 以降は自動で読み込まれます

APIキーは [Google AI Studio](https://aistudio.google.com/apikey) で無料で取得できます。

> **注**: APIキーはこのPC内のファイル（`data/user_settings.yaml`）にのみ保存されます。外部サーバーには送信されません。このファイルは `.gitignore` に登録済みのため、git commitには含まれません。
...
環境変数が設定されている場合は、GUI設定よりも環境変数が優先されます。
...
data/
  ...
  user_settings.yaml    # ユーザー設定（APIキー等、.gitignore対象）
tests/
